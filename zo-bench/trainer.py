...
class OurTrainer(Trainer):
    ...
    def __init__(self, client_id, current_round, evaluate_func, dev_samples, eval_samples, *args, **kwargs):
    ...
    def _inner_training_loop(
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.eval_steps is not None and (total_steps + 1) % self.args.eval_steps == 0:
                    ...
                    if "accuracy" in test_metrics:
                        self.log(self.base_log | {"test_acc": test_metrics["accuracy"], "val_acc": val_metrics["accuracy"]})
                        wandb.log(self.base_log | {"test_acc": test_metrics["accuracy"], "val_acc": val_metrics["accuracy"]})
                    else:
                        keys = list(test_metrics.keys())
                        log_dict = self.base_log
                        for k in keys:
                ...
                self.log(self.base_log | {"peak_mem": max_memory_allocated / 1024 ** 3,
                          "step_consumption": train_step_duration * 1000})
                wandb.log(self.base_log | {"peak_mem": max_memory_allocated / 1024 ** 3,
                           "step_consumption": train_step_duration * 1000})
        ...
        wandb.log(self.base_log | metrics)
        self.log(self.base_log | metrics)
